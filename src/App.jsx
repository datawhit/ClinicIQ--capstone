...
- If you don't have enough info, say so in one sentence.`,
  ...
  const stats = [
    { label:"Total Patients", value:patients.length, color:NYU.purple },
    { label:"Active", value:patients.filter(p=>calculateStatus(p)==="Active").length, color:NYU.green },
    { label:"Needs Attention", value:urgentPatients.length, color:NYU.orange },
    { label:"Complete", value:patients.filter(p=>calculateStatus(p)==="Treatment Complete").length, color:NYU.purpleMid },
    { label:"Pre-Auth Alerts", value:allPreAuthNudges.length+allLabNudges.length, color:NYU.blue },
  ];
      ...
      <>
        ...
        <div style={{ minHeight:"100vh", background:"linear-gradient(135deg, #f8f6fb 0%, #ede8f5 100%)", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ textAlign:"center" }}>
            <div style={{ width:48, height:48, borderRadius:14, background:`linear-gradient(135deg, ${NYU.purple}, ${NYU.accent})`, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px" }}>
          ...
          <div style={{ width:"100%", maxWidth:420 }}>
            ...
            <div style={{ background:"white", borderRadius:24, padding:32, boxShadow:"0 8px 40px rgba(107,33,168,0.1)", border:`1px solid ${NYU.gray100}` }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                ...
                <button className="action-btn" onClick={handleLogin} disabled={loginLoading} style={{ background:NYU.purple, color:"white", width:"100%", marginTop:4, padding:"13px 20px", fontSize:15, opacity:loginLoading?0.7:1 }}>{loginLoading?<><span className="spinner"/>Signing in…</>:"Sign In"}</button>
              </div>
      ...
      <div style={{ minHeight:"100vh", background:NYU.gray50 }}>
        ...
        <div className="page-inner" style={{ maxWidth:1100, margin:"0 auto", padding:"28px 32px" }}>
          ...
          {tab==="calendar"&&(()=>{
            ...
            const apptMap={};
            patients.forEach(p=>{ if(p.nextAppt){ if(!apptMap[p.nextAppt]) apptMap[p.nextAppt]=[]; apptMap[p.nextAppt].push({ type:"confirmed",patient:p,duration:PROCEDURE_DURATIONS[p.procedure]||PROCEDURE_DURATIONS.default,label:p.alias,sublabel:p.procedure||p.discipline,time:p.nextApptTime||"",color:NYU.purple,bg:NYU.purpleLight }); } });
            patients.filter(p=>!p.treatmentComplete&&!p.nextAppt).forEach(p=>{ const pred=predictCompletion(p); if(!pred||!p.lastVisit) return; const avgInterval=pred.avgInterval||21; const suggestedDate=fmt(addDays(new Date(p.lastVisit),avgInterval)); if(!apptMap[suggestedDate]) apptMap[suggestedDate]=[]; apptMap[suggestedDate].push({ type:"suggested",patient:p,duration:PROCEDURE_DURATIONS[p.procedure]||PROCEDURE_DURATIONS.default,label:`Schedule ${p.alias}`,sublabel:`Suggested · ${p.discipline}`,time:"",color:NYU.purpleMid,bg:"#f3e8ff" }); });
            ...
            return (
              <div>
                ...
                <div style={{ display:"flex",gap:16,marginBottom:16,flexWrap:"wrap",alignItems:"center" }}>
                  {[
                    { color:NYU.purple, label:"Confirmed" },
                    { color:NYU.purpleMid, label:"Suggested" },
                ...
                {calView==="week"&&(
                  <div style={{ background:"white",borderRadius:16,border:`1px solid ${NYU.gray100}`,overflow:"hidden" }}>
                    <div style={{ display:"grid",gridTemplateColumns:"64px repeat(7,1fr)",borderBottom:`1px solid ${NYU.gray100}` }}>
                      ...
                      {weekDays.map((d,i)=>(
                        <div key={i} style={{ padding:"10px 12px",borderRight:i<6?`1px solid ${NYU.gray100}`:"none",background:isToday(d)?NYU.lavender:"white" }}>
                          ...
                          <div style={{ fontSize:20,fontWeight:700,color:isToday(d)?NYU.purple:NYU.gray900,fontFamily:"'Fraunces', serif",lineHeight:1.2 }}>{d.getDate()}</div>
                      ...
                      {Array(totalDays).fill(null).map((_,i)=>{
                        ...
                        return (
                          <div key={dayNum}
                            ...
                            <div style={{ fontSize:13,fontWeight:700,color:todayFlag?NYU.purple:weekend?NYU.gray400:NYU.gray900,marginBottom:4,width:24,height:24,borderRadius:"50%",background:todayFlag?NYU.lavender:"transparent",display:"flex",alignItems:"center",justifyContent:"center" }}>{dayNum}</div>
                            ...
                            {dayAppts.length>2&&<div style={{ fontSize:10,color:NYU.purple,fontWeight:600,marginTop:2 }}>+{dayAppts.length-2} more</div>}
                          </div>
                        );
                      })}
                    </div>
                                ...
                                <div key={i} style={{ display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<appts.length-1?`1px solid ${NYU.gray100}`:"none" }}>
                                  ...
                                  <div style={{ flex:1 }}>
                                    <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                                      ...
                                      {appt.time&&<span style={{ fontSize:11,fontWeight:700,color:isConflict?NYU.red:NYU.purple,background:isConflict?"#fee2e2":NYU.lavender,borderRadius:99,padding:"2px 8px" }}>{isConflict?"⚠ ":""}{appt.time}</span>}
                                    </div>
                                    <div style={{ fontSize:12,color:NYU.gray400,marginTop:2 }}>{appt.sublabel} · {appt.duration} min</div>
                                  </div>
                                  <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                                    <span style={{ fontSize:10,fontWeight:600,color:appt.type==="suggested"?NYU.purpleMid:NYU.green,background:appt.type==="suggested"?"#f3e8ff":NYU.greenLight,borderRadius:99,padding:"2px 8px" }}>{appt.type==="suggested"?"Suggested":"Confirmed"}</span>
                                    {appt.type==="confirmed"&&(
                                    ...
                                    {appt.type==="suggested"&&(
                                      <button onClick={()=>{ setApptModalMode("new"); setApptDraft({ patientId:appt.patient.id, date:appt.patient.nextAppt||date, time:"" }); setShowApptModal(true); }}
                                        style={{ background:NYU.lavender,border:"none",borderRadius:8,padding:"4px 8px",cursor:"pointer",fontSize:11,color:NYU.purple,fontFamily:"'Inter', sans-serif",fontWeight:600 }}>+ Confirm</button>
                                    )}
                                  </div>
                                </div>
                              );
              ...
              <div style={{ flex:1,minWidth:0 }}>
                {!activeNote||!noteDraft?(
                  <div style={{ background:"white",borderRadius:16,border:`1px solid ${NYU.gray100}`,padding:"80px 40px",textAlign:"center" }}>
                    ...
                    <button className="action-btn" onClick={async()=>{ const id=`n${Date.now()}`; const todayStr=new Date().toISOString().split("T")[0]; const n={ id,title:"Untitled",body:"",category:"General",pinned:false,updatedAt:todayStr }; await fetch("/api/notes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)}); setNotes(prev=>[n,...prev]); setActiveNote(id); setNoteDraft(n); }} style={{ background:NYU.purple,color:"white" }}>Create your first note</button>
                  </div>
                ):(
                  <div style={{ background:"white",borderRadius:16,border:`1px solid ${NYU.gray100}`,overflow:"hidden" }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
              <div style={{ background:NYU.lavender,borderRadius:16,padding:"20px 24px",marginBottom:12,display:"flex",alignItems:"center",justifyContent:"space-between",border:`1px solid ${NYU.gray200}` }}>
                ...
                <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                  ...
                  {!editingGoals?(
                    <button className="action-btn" onClick={()=>{ setEditGoalsDraft([...customGoals.map(g=>({...g}))]); setEditingGoals(true); }} style={{ background:"white",color:NYU.purple,fontSize:13 }}>✎ Edit Goals</button>
                  ):(
                    <div style={{ display:"flex",gap:8 }}>
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
                  {editGoalsDraft.map((goal,index)=>(
                    <div key={goal.discipline+index} draggable onDragStart={()=>setDragIndex(index)} onDragOver={e=>e.preventDefault()} onDrop={()=>{ if(dragIndex===null||dragIndex===index) return; const updated=[...editGoalsDraft]; const [moved]=updated.splice(dragIndex,1); updated.splice(index,0,moved); setEditGoalsDraft(updated); setDragIndex(null); }}
                      className="card" style={{ padding:"14px 16px",display:"flex",alignItems:"center",gap:12,opacity:goal.visible?1:0.45,cursor:"grab",border:dragIndex===index?`2px dashed ${NYU.purple}`:"none" }}>
                      <span style={{ fontSize:18,color:NYU.gray400,cursor:"grab",userSelect:"none" }}>⠿</span>
                      <input type="checkbox" checked={goal.visible} onChange={()=>{ const updated=[...editGoalsDraft]; updated[index]={...updated[index],visible:!updated[index].visible}; setEditGoalsDraft(updated); }} style={{ width:16,height:16,accentColor:NYU.purple,cursor:"pointer" }}/>
                      ...
                      <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                        ...
                        <input type="number" min={1} max={99} value={goal.required} onChange={e=>{ const updated=[...editGoalsDraft]; updated[index]={...updated[index],required:parseInt(e.target.value)||1}; setEditGoalsDraft(updated); }} style={{ width:56,padding:"5px 8px",borderRadius:8,border:`1.5px solid ${NYU.gray200}`,fontSize:14,fontWeight:600,color:NYU.purple,textAlign:"center",fontFamily:"'Inter', sans-serif",outline:"none" }}/>
                      </div>
                    </div>
                  ))}
                  <div className="card" style={{ padding:"14px 16px",display:"flex",alignItems:"center",gap:10,border:`1.5px dashed ${NYU.gray200}`,boxShadow:"none" }}>
                    ...
                    <button className="action-btn" onClick={()=>{ if(!newDisciplineName.trim()) return; setEditGoalsDraft([...editGoalsDraft,{ discipline:newDisciplineName.trim(),required:5,visible:true,order:editGoalsDraft.length }]); setNewDisciplineName(""); }} style={{ background:NYU.purple,color:"white",padding:"7px 14px" }}>Add</button>
                  </div>
                </div>
              )}
                  ...
                  <div key={goal.discipline} className="card" style={{ padding:"16px 20px" }}>
                    ...
                    {qualifyingPatients.length>0&&(
                      <div style={{ marginTop:10,fontSize:12,color:NYU.gray600 }}>
                        <span style={{ fontWeight:600,color:NYU.purple }}>Active: </span>
        ...
        <div style={{ position:"fixed",inset:0,zIndex:800,background:"#f8f6fb",overflowY:"auto",animation:"slideInRight 0.25s ease" }}>
          ...
          <div style={{ maxWidth:1100,margin:"0 auto",padding:"28px 32px 80px" }}>
            ...
            <div style={{ display:"flex",gap:8,marginBottom:20,flexWrap:"wrap" }}>
              {["All","Active","Urgent","F/U Needed","Complete"].map(f=>(<button key={f} className="filter-btn" onClick={()=>setFilter(f)} style={{ background:filter===f?NYU.purple:NYU.gray100,color:filter===f?"white":NYU.gray600,border:"none",fontSize:12,padding:"6px 14px" }}>{f}</button>))}
            </div>
            <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
              {filtered.map(patient=>{
                const status=calculateStatus(patient);
              ...
              {customGoals.filter(g=>g.visible).map(goal=>{
                ...
                return (
                  <div key={goal.discipline} className="card" style={{ padding:"16px 20px" }}>
                    ...
                    <div className="progress-bar"><div className="progress-fill" style={{ width:`${pct}%`,background:color }}/></div>
                    {activePts.length>0&&<div style={{ marginTop:10,fontSize:12,color:NYU.gray600 }}><span style={{ fontWeight:600,color:NYU.purple }}>Active: </span>{activePts.map(p=>p.alias).join(", ")}</div>}
                  </div>
                );
              })}
            </div>
                  ...
                  return (
                    <div key={patient.id} style={{ background:"white",borderRadius:16,border:`1px solid #fed7aa`,overflow:"hidden" }}>
                      <div style={{ padding:"16px 20px",display:"flex",alignItems:"center",gap:14 }}>
                        ...
                        <button className="action-btn" onClick={()=>{ setDetailPatient(patient.id); setShowUrgentPanel(false); }} style={{ background:NYU.purple,color:"white",fontSize:12,padding:"7px 16px" }}>View →</button>
                      </div>
        ...
        return (
          <div style={{ position:"fixed",inset:0,zIndex:800,background:"#f8f6fb",overflowY:"auto",animation:"slideInRight 0.25s ease" }}>
            ...
            <div style={{ maxWidth:1100,margin:"0 auto",padding:"28px 32px 80px" }}>
              ...
              <div style={{ background:NYU.lavender,borderRadius:14,padding:"12px 18px",marginBottom:16 }}>
                <div style={{ fontSize:11,fontWeight:600,color:NYU.purple,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:2 }}>Axium Chart Number <span style={{ fontSize:10,fontWeight:400,color:NYU.gray400 }}>(student reference only — not displayed externally)</span></div>
                ...
                <div style={{ background:"white",borderRadius:16,padding:"18px 20px",marginBottom:16,border:`1px solid ${NYU.gray100}` }}>
                  ...
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:0 }}>
                    ...
                    <div>
                      ...
                      <input type="date" value={patient.nextAppt||""} onChange={e=>updateField(patient.id,"nextAppt",e.target.value||null)} style={{ ...inputStyle,fontSize:13,padding:"8px 10px",color:patient.nextAppt?NYU.purple:NYU.gray400 }}/>
                      ...
                      {prediction&&prediction.daysOut>0&&!patient.nextAppt&&(
                        <button onClick={()=>updateField(patient.id,"nextAppt",prediction.date)}
                          style={{ marginTop:6,fontSize:11,fontWeight:600,color:NYU.purple,background:NYU.lavender,border:`1px solid ${NYU.gray200}`,borderRadius:99,padding:"4px 12px",cursor:"pointer",fontFamily:"'Inter', sans-serif",display:"flex",alignItems:"center",gap:5,width:"100%" }}>
                          ...
                          {selectedVisit===i&&(
                            <div style={{ marginTop:12,paddingTop:12,borderTop:`1px solid ${NYU.gray200}`,animation:"slideUp 0.15s ease" }}>
                              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:v.notes?10:0 }}>
                                ...
                                {v.cdtCode&&<div><div style={{ fontSize:10,color:NYU.gray400,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:3 }}>CDT Code</div><div style={{ fontSize:13,fontWeight:500,color:NYU.purple }}>{v.cdtCode}</div></div>}
                              </div>
              ...
              <div style={{ background:"white",borderRadius:16,padding:"18px 20px",marginBottom:16,border:`1px solid ${NYU.gray100}` }}>
                ...
                {patient.patientLanguage&&patient.patientLanguage!=="English"&&(
                  <div style={{ background:NYU.lavender,borderRadius:10,padding:"10px 14px",fontSize:12,color:NYU.gray700,lineHeight:1.5 }}>
          ...
          <div style={{ position:"absolute",bottom:72,right:0,width:380,height:540,background:"white",borderRadius:20,boxShadow:"0 8px 40px rgba(87,6,140,0.22)",display:"flex",flexDirection:"column",overflow:"hidden",animation:"slideUp 0.2s ease" }}>
            ...
            <div style={{ flex:1,overflowY:"auto",padding:"14px",display:"flex",flexDirection:"column",gap:10 }}>
              {chatMessages.map((msg,i)=>(
                <div key={i} style={{ display:"flex",justifyContent:msg.role==="user"?"flex-end":"flex-start",alignItems:"flex-end",gap:8 }}>
                  ...
                  <div style={{ maxWidth:"82%", display:"flex", flexDirection:"column", gap:6, alignItems:msg.role==="user"?"flex-end":"flex-start" }}>
                    <div style={{ padding:"10px 14px",borderRadius:msg.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px",background:msg.role==="user"?NYU.purple:NYU.gray50,color:msg.role==="user"?"white":NYU.gray900,fontSize:13,lineHeight:1.5,border:msg.role==="assistant"?`1px solid ${NYU.gray100}`:"none" }}>
                      {msg.content}
                    </div>
                    {/* Smart action button */}
                    {msg.action&&(()=>{
                      ...
                      if(type==="patient"||type==="logvisit"){
                        ...
                        return (
                          <button onClick={()=>{ setDetailPatient(p.id); if(type==="logvisit"){ setShowLogModal(p.id); setNewVisit({ date:"",procedure:"",notes:"",nextAppt:"",cdtCode:"" }); setNlpInput(""); setNlpParsed(null); setNlpError(""); } }}
                            style={{ fontSize:11,fontWeight:600,color:NYU.purple,background:NYU.lavender,border:`1px solid ${NYU.gray200}`,borderRadius:99,padding:"5px 12px",cursor:"pointer",fontFamily:"'Inter', sans-serif",display:"flex",alignItems:"center",gap:5 }}>
              ...
              {chatLoading&&(
                <div style={{ display:"flex",alignItems:"flex-end",gap:8 }}>
                  <div style={{ width:24,height:24,borderRadius:6,background:NYU.accent,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}><span style={{ color:"white",fontSize:11 }}>✦</span></div>
                  <div style={{ background:NYU.gray50,border:`1px solid ${NYU.gray100}`,borderRadius:"16px 16px 16px 4px",padding:"10px 14px",display:"flex",gap:4,alignItems:"center" }}>
                    {[0,1,2].map(i=><div key={i} style={{ width:6,height:6,borderRadius:"50%",background:NYU.purple,opacity:0.5,animation:`bounce ${0.5+i*0.15}s ease-in-out infinite alternate` }}/>)}
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ padding:"12px 14px",borderTop:`1px solid ${NYU.gray100}`,display:"flex",gap:8,alignItems:"flex-end" }}>
              ...
              <button onClick={sendChat} disabled={chatLoading||!chatInput.trim()}
                style={{ background:NYU.purple,border:"none",borderRadius:10,width:36,height:36,cursor:"pointer",color:"white",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,opacity:(!chatInput.trim()||chatLoading)?0.5:1 }}>↑</button>
            </div>
          </div>
        )}
        <button onClick={()=>setChatOpen(!chatOpen)} style={{ width:56,height:56,borderRadius:"50%",background:`linear-gradient(135deg, ${NYU.purple}, ${NYU.accent})`,border:"none",cursor:"pointer",boxShadow:"0 4px 20px rgba(137,0,225,0.4)",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s",transform:chatOpen?"rotate(45deg) scale(0.95)":"scale(1)" }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
              ...
              <div style={{ display:"flex",gap:10,marginTop:4 }}>
                ...
                <button className="action-btn"
                  ...
                  style={{ flex:1,background:NYU.purple,color:"white" }}>
                ...
                <div>
                  ...
                  <div style={{ display:"flex",background:NYU.gray100,borderRadius:10,padding:3,gap:2 }}>
                    {[{ key:"me",label:"👤 My View" },{ key:"partner",label:`🤝 @${partnerName}` }].map(v=>(
                      <button key={v.key} onClick={()=>setPairedViewingAs(v.key)} style={{ flex:1,padding:"7px 14px",borderRadius:8,border:"none",cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"'Inter', sans-serif",background:pairedViewingAs===v.key?"white":"transparent",color:pairedViewingAs===v.key?NYU.purple:NYU.gray400,boxShadow:pairedViewingAs===v.key?"0 1px 4px rgba(107,33,168,0.1)":"none",transition:"all 0.15s" }}>{v.label}</button>
                    ))}
                  </div>
                    ...
                    {sharedPatients.map(p=>{
                      ...
                      return (
                        <div key={p.id} style={{ background:"white",border:`1px solid ${NYU.gray100}`,borderRadius:16,padding:"16px 18px",marginBottom:12 }}>
                          ...
                          {p.handoffNotes&&<div style={{ padding:"10px 12px",background:NYU.lavender,borderRadius:10,fontSize:12,color:NYU.gray700,fontStyle:"italic",marginTop:8 }}>📌 "{p.handoffNotes}"</div>}
                        </div>
                      );
                    })}
                  </div>
              ...
              return (
                <div>
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:24 }}>
                    {[{ label:"Shared",value:sharedPatients.length,color:NYU.green },{ label:"Partners",value:partners.length,color:NYU.purple },{ label:"Needs Partner",value:needsPartner.length,color:NYU.orange }].map(s=>(
                  ...
                  {partners.length>0?partners.map(partner=>(
                    <div key={partner.name} style={{ marginBottom:24 }}>
                      <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:12 }}>
                        <div style={{ width:38,height:38,borderRadius:10,background:NYU.purple,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}><span style={{ fontSize:15,color:"white",fontWeight:700 }}>{partner.name.charAt(0)}</span></div>
                        <div><div style={{ fontWeight:700,fontSize:15,color:NYU.gray900 }}>{partner.name}</div><div style={{ fontSize:12,color:NYU.purple }}>{partner.year} · {partner.patients.length} shared</div></div>
                      </div>
                          ...
                          <div key={p.id} style={{ background:"white",border:`1px solid ${NYU.gray100}`,borderRadius:14,padding:"14px 16px",marginBottom:8 }}>
                            ...
                            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>
                              {[{ key:"shareCalendar",label:"📅 Next Appt",value:p.nextAppt||"Not scheduled" },{ key:"shareVisitHistory",label:"📋 Visits",value:`${p.visitLog?.length||0} logged` },{ key:"shareTreatmentNotes",label:"📝 Procedure",value:p.procedure||"None" },{ key:"sharePreAuth",label:"📄 Pre-Auth",value:p.preAuth }].map(({ key,label,value })=>{ const shared=p[key]!==false; return (
                                <div key={key} onClick={()=>updateField(p.id,key,!shared)} style={{ padding:"8px 10px",background:shared?NYU.lavender:NYU.gray50,borderRadius:8,cursor:"pointer",border:`1px solid ${shared?NYU.purple+"22":NYU.gray100}` }}>
                            ...
                            {p.handoffNotes&&<div style={{ marginTop:8,padding:"8px 10px",background:NYU.lavender,borderRadius:8,fontSize:12,color:NYU.gray700,fontStyle:"italic" }}>"{p.handoffNotes}"</div>}
                          </div>
                        );
                      })}
                    </div>
          ...
          <div className="modal-box" onClick={e=>e.stopPropagation()} style={{ maxWidth:440 }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:20 }}>
                  ...
                  {["monday","tuesday","wednesday","thursday","friday","saturday","sunday"].map(day=>{
                    ...
                    return (
                      <div key={day} style={{ display:"flex",alignItems:"center",gap:10 }}>
                        <div onClick={()=>setSettingsDraft(p=>({ ...p,clinicSchedule:{ ...p.clinicSchedule,[day]:{ ...sched,enabled:!sched.enabled } } }))} style={{ width:36,height:20,borderRadius:99,background:sched.enabled?NYU.purple:NYU.gray200,cursor:"pointer",position:"relative",flexShrink:0,transition:"background 0.2s" }}>
              ...
              <div style={{ background:NYU.gray50,borderRadius:14,padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                ...
                <div style={{ width:36,height:36,borderRadius:10,background:`linear-gradient(135deg, ${NYU.purple}, ${NYU.accent})`,display:"flex",alignItems:"center",justifyContent:"center" }}><span style={{ color:"white",fontSize:16,fontWeight:700 }}>C</span></div>
              </div>
            </div>
            <div style={{ display:"flex",gap:10,marginTop:24 }}>
              ...
              <button className="action-btn" onClick={async()=>{
                ...
                try {
                  await Promise.all([
                    fetch("/api/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({graduationDate:settingsDraft.graduationDate,customGoals,clinicSchedule:settingsDraft.clinicSchedule||clinicSchedule})}),
