...
- **Mobile Responsiveness** — `.tab-nav` scrolls horizontally with `nowrap`; `.top-bar-name` hidden on small screens; `.chat-panel` and `.quick-log-panel` resize to viewport width; floating buttons repositioned to `bottom:16px` on mobile; page inner padding adds bottom space; `.intel-row` stacks vertically on mobile; all touch targets ≥44px
